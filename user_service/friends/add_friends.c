...
void add_friends(char sender_name[]){
    int c;
    while ((c = getchar()) != '\n' && c != EOF); // remove the newline that was entered before the function call
    
    char users_tba[256]; // users to be added
    printf("Enter usernames to add (in one line separated by space): ");
    fgets(users_tba, sizeof(users_tba), stdin); // get users to be added
    users_tba[strcspn(users_tba, "\n")] = '\0'; // remove the new line char
    char *receiver_name = strtok(users_tba, " "); // split the users based on token of space

    
    while (receiver_name != NULL) {
        // check if user add self
        if (strcmp(sender_name, receiver_name) == 0) {
            printf("You cannot send a friend request to yourself.\n");
            receiver_name = strtok(NULL, " ");
            continue;
        }
